...
namespace PMT01700FRONT
{
    public partial class PMT01700LOO_OfferList : R_Page, R_ITabPage
    {
        ...
        private int _pageSizeOfferList = 10;
        private int _pageSizeUnitList = 5;
        #endregion
        ...
        private async Task OnChangeLCancelled(bool poParam)
        {
            var loException = new R_Exception();
        ...
        private async Task CancelLOOBtn()
        {
            var loEx = new R_Exception();
            await lockingButton(true);
            try
        ...
        private async Task OnActiveTabIndexChanged(R_TabStripTab eventArgs)
        {
            ...
            {
                case "OfferList":
                    var lcTempCREF_NO = _viewModel.oParameter.CREF_NO;
                    await _gridOfferList.R_RefreshGrid(null);
                    if (!string.IsNullOrEmpty(_viewModel.oParameter.CREF_NO) && _viewModel.loListOfferList.Any())
                    {
                        _viewModel.oParameter.ODataUnitList = null;
                        var loData = _viewModel.loListOfferList
                            .FirstOrDefault(x => x?.CREF_NO == lcTempCREF_NO);

                        if (!string.IsNullOrEmpty(loData.CREF_NO))
                            await _gridOfferList.R_SelectCurrentDataAsync(loData);
                    }
                    break;
                default:
                    break;
        ...
        private async Task BeforeOpen_PopupPrint(R_BeforeOpenPopupEventArgs eventArgs)
        {
            ...
            try
            {
                var loData = (PMT01700LOO_OfferList_OfferListDTO)_gridOfferList.GetCurrentData();
        ...
        private async Task lockingButton(bool param)
        {
            ...
            try
            {
                var loData = R_FrontUtility.ConvertObjectToObject<PMT01700ParameterFrontChangePageDTO>(_conductorOfferList!.R_GetCurrentData());
