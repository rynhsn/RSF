...
{
    public partial class APT00231 : R_Page
    {
        [Inject] private R_ILocalizer<APT00200FrontResources.Resources_Dummy_Class> _localizer { get; set; }

        private APT00231ViewModel loViewModel = new APT00231ViewModel();

        private R_ConductorGrid _conductorRef;

        private R_Grid<APT00231DTO> _gridAdditionalRef;
        ...
        private async Task Additional_ServiceGetRecord(R_ServiceGetRecordEventArgs eventArgs)
        {
            ...
            try
            {
                await loViewModel.GetAdditionalAsync((APT00231DTO)eventArgs.Data);
        ...
        private async Task Additional_ServiceSave(R_ServiceSaveEventArgs eventArgs)
        {
            ...
            try
            {
                await loViewModel.SaveAdditionalAsync((APT00231DTO)eventArgs.Data, (eCRUDMode)eventArgs.ConductorMode);
        ...
        private async Task Additional_ServiceDelete(R_ServiceDeleteEventArgs eventArgs)
        {
            ...
            try
            {
                await loViewModel.DeleteAdditionalAsync((APT00231DTO)eventArgs.Data);
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        {
            if (eventArgs.ColumnName == nameof(APT00231DTO.CADD_DEPT_CODE))
            {
            ...
            else if (eventArgs.ColumnName == nameof(APT00231DTO.CCHARGES_ID))
            {
        ...
        private void Grid_AfterlookUp(R_AfterOpenGridLookupColumnEventArgs eventArgs)
        {
            if (eventArgs.ColumnName == nameof(APT00231DTO.CADD_DEPT_CODE))
            {
                ...
                APT00231DTO loGetData = (APT00231DTO)eventArgs.ColumnData;
