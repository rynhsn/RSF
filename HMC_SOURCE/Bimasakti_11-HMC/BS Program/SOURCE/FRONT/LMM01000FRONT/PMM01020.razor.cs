...
{
    public partial class PMM01020 : R_Page, R_ITabPage
    {
        #region viewModel
        private PMM01020ViewModel _viewModel = new PMM01020ViewModel();
        private PMM01020SaveBatchViewModel _viewModelSave = new PMM01020SaveBatchViewModel();
        private PMM01000UniversalViewModel _Universal_viewModel = new PMM01000UniversalViewModel();
        #endregion

        #region Private Grid Conductor
        private R_Conductor _RateWG_conductorRef;
        private R_ConductorGrid _RateWGDetail_conductorRef;
        private R_Grid<PMM01021DTO> _RateWGDetail_gridRef;
        private R_Grid<PMM01020DTO> _RateWG_gridRef;
        private PMM01020DTO _HeaderData = new PMM01020DTO();
        ...
        protected override async Task R_Init_From_Master(object poParameter)
        {
            ...
            try
            {
                ...
                PMM01020DTO loParam;
                loParam = R_FrontUtility.ConvertObjectToObject<PMM01020DTO>(poParameter);
                _HeaderData = loParam;
                await _viewModel.GetProperty(loParam);

                await _RateWG_gridRef.R_RefreshGrid(loParam);
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        protected async override Task<bool> R_LockUnlock(R_LockUnlockEventArgs eventArgs)
        {
            ...
            try
            {
                var loData = (PMM01020DTO)eventArgs.Data;
                ...
                if (eventArgs.Mode == R_eLockUnlock.Lock)
                {
                    var loLockPar = new R_ServiceLockingLockParameterDTO
                    {
                        ...
                        Program_Id = "PMM01020",
                        Table_Name = "PMM_UTILITY_RATE_WG_HD",
        ...
        #region Header Form
        private async Task RateWGDate_ServiceGetListRecord(R_ServiceGetListRecordEventArgs eventArgs)
        {
            var loEx = new R_Exception();

            try
            {
                var loParam = R_FrontUtility.ConvertObjectToObject<PMM01020DTO>(eventArgs.Parameter);
                await _viewModel.GetRateWGList(loParam);

                eventArgs.ListEntityResult = _viewModel.RateWGList;
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        private async Task RateWG_ServiceGetRecord(R_ServiceGetRecordEventArgs eventArgs)
        {
            ...
            try
            {
                await _viewModel.GetRateWG((PMM01020DTO)eventArgs.Data);
            ...
            if (eventArgs.ConductorMode == R_BlazorFrontEnd.Enums.R_eConductorMode.Normal)
            {
                ...
                if (loData != null)
                {
                    if (string.IsNullOrWhiteSpace(loData.CCHARGES_DATE) == false)
                        await _RateWGDetail_gridRef.R_RefreshGrid(loData);
                }
            }
        }
        private async Task RateWG_Validation(R_ValidationEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (loData.CUSAGE_RATE_MODE == "HM" && _RateWGDetail_gridRef.DataSource.Count <= 0)
                {
                    loEx.Add(R_FrontUtility.R_GetError(
                        typeof(PMM01000FrontResources.Resources_Dummy_Class),
                        "10021"));
        ...
        private async Task RateWG_ServiceSave(R_ServiceSaveEventArgs eventArgs)
        {
            ...
            {
                var loData = (PMM01020DTO)eventArgs.Data;
                loData.CRATE_WG_LIST = _RateWGDetail_gridRef.DataSource.ToList();
                loData.CCHARGES_DATE = loData.DCHARGES_DATE.Value.ToString("yyyyMMdd");
                await _viewModelSave.SaveDeleteRateWG(loData, (eCRUDMode)eventArgs.ConductorMode, clientHelper.CompanyId, clientHelper.UserId);
        ...
        private async Task RateWG_ServiceDelete(R_ServiceDeleteEventArgs eventArgs)
        {
            var loEx = new R_Exception();
            ...
            try
            {
                var loData = (PMM01020DTO)eventArgs.Data;
                loData.CRATE_WG_LIST = _RateWGDetail_gridRef.DataSource.ToList();
                await _viewModelSave.SaveDeleteRateWG(loData, eCRUDMode.DeleteMode, clientHelper.CompanyId, clientHelper.UserId);
                if (_RateWGDetail_gridRef.DataSource.Count > 0)
                {
        ...
        private void RateWG_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            var loData = (PMM01020DTO)eventArgs.Data;
        ...
        private async Task RateWGDetail_ServiceGetListRecord(R_ServiceGetListRecordEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                {
                    var loEventParam = (PMM01020DTO)eventArgs.Parameter;
                    var loParam = R_FrontUtility.ConvertObjectToObject<PMM01021DTO>(loEventParam);
        ...
        private async Task RateWGDetail_Validation(R_ValidationEventArgs eventArgs)
        {
            ...
            try
            {
                var loData = (PMM01021DTO)eventArgs.Data;
                var loListInt = _RateWGDetail_gridRef.DataSource.Where(x => x.SEQ_NO != loData.SEQ_NO).Select(x => x.IUP_TO_USAGE).ToList();
                ...
                if (loListInt.Count > 0)
                {
                    ...
                    if (lCancel)
                    {
                        loEx.Add("10026", string.Format(_localizer["10026"], _localizer["_UsageM3"], loListInt.Max()));
                    }
                }
        ...
        private void RateWGDetail_AfterAdd(R_AfterAddEventArgs eventArgs)
        {
            var loParentData = (PMM01020DTO)_RateWG_conductorRef.R_GetCurrentData();
            var loData = (PMM01021DTO)eventArgs.Data;
            loData.SEQ_NO = _RateWGDetail_gridRef.DataSource.Count + 1;
        ...
        private async Task Currency_OnLostFocus()
        {
            ...
            try
            {
                var loData = (PMM01020DTO)_RateWG_conductorRef.R_GetCurrentData();
        ...
        public async Task RefreshTabPageAsync(object poParam)
        {
            ...
            try
            {
                var loParam = R_FrontUtility.ConvertObjectToObject<PMM01020DTO>(poParam);
                ...
                if (string.IsNullOrWhiteSpace(loParam.CCHARGES_ID))
                {
                    ...
                    _viewModel.R_SetCurrentData(new PMM01020DTO());
                    if (_RateWG_gridRef.DataSource.Count > 0)
                    {
                        _RateWG_gridRef.DataSource.Clear();
                    }
                    if (_RateWGDetail_gridRef.DataSource.Count > 0)
                    {
                        _RateWGDetail_gridRef.DataSource.Clear();
        ...
        public async Task Button_OnClickPrintAsync()
        {
            ...
            {
                // set Param
                var loParam = R_FrontUtility.ConvertObjectToObject<PMM01020PrintParamDTO>(_viewModel.RateWG);
                ...
                if (loValidate == R_eMessageBoxResult.Yes)
                {
                    await _reportService.GetReport(
                     ...
                     "PM",
                     "rpt/PMM01020Print/AllRateWGReportPost",
                     "rpt/PMM01020Print/AllStreamRateWGReportGet",
                     loParam);
                }
            }
            catch (Exception ex)
