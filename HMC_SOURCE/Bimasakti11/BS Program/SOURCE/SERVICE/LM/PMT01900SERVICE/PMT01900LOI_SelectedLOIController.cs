...
namespace PMT01900Service
{
    ...
    [ApiController]
    public class PMT01900LOI_SelectedLOIController : ControllerBase, IPMT01900LOI_SelectedLOI
    {
        private readonly LoggerPMT01900? _loggerPMT01100;
        private readonly ActivitySource _activitySource;

        public PMT01900LOI_SelectedLOIController(ILogger<PMT01900LOI_SelectedLOIController> logger)
        {
            LoggerPMT01900.R_InitializeLogger(logger);
            _loggerPMT01100 = LoggerPMT01900.R_GetInstanceLogger();
            _activitySource = PMT01900Activity.R_InitializeAndGetActivitySource(nameof(PMT01900LOI_SelectedLOIController));
        }


        [HttpPost]
        public R_ServiceGetRecordResultDTO<PMT01900LOI_SelectedLOIDTO> R_ServiceGetRecord(R_ServiceGetRecordParameterDTO<PMT01900LOI_SelectedLOIDTO> poParameter)
        {
            string? lcMethod = nameof(R_ServiceGetRecord);
            _loggerPMT01100.LogInfo(string.Format("Start Method {0}", lcMethod));
            var loEx = new R_Exception();
            var loRtn = new R_ServiceGetRecordResultDTO<PMT01900LOI_SelectedLOIDTO>();

            try
            {
                _loggerPMT01100.LogInfo(string.Format("Initialize the loCls in method {0}", lcMethod));
                var loCls = new PMT01900LOI_SelectedLOICls();
        ...
        [HttpPost]
        public R_ServiceSaveResultDTO<PMT01900LOI_SelectedLOIDTO> R_ServiceSave(R_ServiceSaveParameterDTO<PMT01900LOI_SelectedLOIDTO> poParameter)
        {
            string? lcMethod = nameof(R_ServiceSave);
            using Activity activity = _activitySource.StartActivity(lcMethod)!;
            _loggerPMT01100.LogInfo(string.Format("Start Method {0}", lcMethod));
            R_Exception? loException = new R_Exception();
            R_ServiceSaveResultDTO<PMT01900LOI_SelectedLOIDTO> loReturn = new();
            try
            {
                _loggerPMT01100.LogInfo(string.Format("Initialize the loCls object as a new instance of Cls in method {0}", lcMethod));
                PMT01900LOI_SelectedLOICls? loCls = new();
        ...
        [HttpPost]
        public R_ServiceDeleteResultDTO R_ServiceDelete(R_ServiceDeleteParameterDTO<PMT01900LOI_SelectedLOIDTO> poParameter)
        {
