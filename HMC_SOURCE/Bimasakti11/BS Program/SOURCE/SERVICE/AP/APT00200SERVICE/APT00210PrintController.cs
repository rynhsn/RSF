...
{
    public class APT00210PrintController : R_ReportControllerBase
    {
        private R_ReportFastReportBackClass _ReportCls;
        private APT00210PrintReportParameterDTO _AllReportParameter;
        private LoggerAPT00210Print _logger;
        private readonly ActivitySource _activitySource;

        #region instantiate
        public APT00210PrintController(ILogger<APT00210PrintController> logger)
        {
            LoggerAPT00210Print.R_InitializeLogger(logger);
            _logger = LoggerAPT00210Print.R_GetInstanceLogger();
            _activitySource = APT00210PrintActivitySourceBase.R_InitializeAndGetActivitySource(nameof(APT00210PrintController));
        ...
        private void _ReportCls_R_InstantiateMainReportWithFileName(ref string pcFileTemplate)
        {
            ...
            pcFileTemplate = System.IO.Path.Combine("Reports", "APT00210PrintReport.frx");
        ...
        [HttpPost]
        public R_DownloadFileResultDTO PrintReportPost(APT00210PrintReportParameterDTO poParameter)
        {
            ...
            APT00210PrintLogKeyDTO loCache = null;
            try
            {
                loRtn = new R_DownloadFileResultDTO();

                loCache = new APT00210PrintLogKeyDTO
                {
                    poParam = poParameter,
                ...
                R_DistributedCache.R_Set(loRtn.GuidResult, R_NetCoreUtility.R_SerializeObjectToByte<APT00210PrintLogKeyDTO>(loCache));
            }
            catch (Exception ex)
            {
        ...
        public FileStreamResult PrintReportGet(string pcGuid)
        {
            ...
            R_Exception loException = new R_Exception();
            APT00210PrintLogKeyDTO loResultGUID = null;
            FileStreamResult loRtn = null;
            try
            {
                //Get Parameter
                loResultGUID = R_NetCoreUtility.R_DeserializeObjectFromByte<APT00210PrintLogKeyDTO>(R_DistributedCache.Cache.Get(pcGuid));
        ...
        #region Helper
        private APT00210PrintReportResultWithBaseHeaderPrintDTO GenerateDataPrint(APT00210PrintReportParameterDTO poParam)
        {
            ...
            R_Exception loException = new R_Exception();
            APT00210PrintReportResultWithBaseHeaderPrintDTO loRtn = new APT00210PrintReportResultWithBaseHeaderPrintDTO();
            List<APT00210PrintReportSubDetailDTO> loSubDetail = new List<APT00210PrintReportSubDetailDTO>();
            try
            {
                APT00210Cls loCls = new APT00210Cls(_logger);

                List<APT00210PrintReportDTO> loCollection = loCls.GetPrintReportList(poParam);

                _logger.LogInfo("Group Data || GenerateDataPrint(Controller)");
                APT00210PrintReportHeaderDTO loTempData = loCollection
                .GroupBy(item => new {
                    item.CREF_NO,
                    item.CREF_DATE,
                    item.CDOC_NO,
                ...
                .Select(header => new APT00210PrintReportHeaderDTO
                {
                    ...
                    FooterData = new APT00210PrintReportFooterDTO()
                    {
                    ...
                    DetailData = header
                        .Select(detail => new APT00210PrintReportDetailDTO
                        {
                            INO = detail.INO,
                ...
                if (loTempData != null)
                {
                    if (!string.IsNullOrWhiteSpace(loTempData.CJRN_ID))
                    {
                        ...
                        APT00210PrintReportSubDetailParameterDTO loSubParameter = new APT00210PrintReportSubDetailParameterDTO()
                        {
                ...
                BaseHeaderDTO loParam = new BaseHeaderDTO()
                {
                    ...
                    CPRINT_NAME = "PURCHASE PurchaseReturn",
                    CUSER_ID = loBaseHeader.USER_ID,
                };
                APT00210PrintReportDTO loGetLogo = loCls.GetBaseHeaderLogoCompany(poParam);
                loParam.BLOGO_COMPANY = loGetLogo.OLOGO;

                _logger.LogInfo("Set Parameter || GenerateDataPrint(Controller)");
                APT00210PrintReportResultDTO loData = new APT00210PrintReportResultDTO()
                {
                    Column = new APT00210PrintReportColumnDTO(),
                    Data = loTempData,
