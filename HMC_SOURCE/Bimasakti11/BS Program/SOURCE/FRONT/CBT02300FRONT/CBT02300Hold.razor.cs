...
{
    public partial class CBT02300Hold
    {
        ...
        {
            await R_MessageBox.Show("", $"Hold cheque(s) processed successfully!”", R_eMessageBoxButtonType.OK);
        }
        ...
        private async Task GetListRecord(R_ServiceGetListRecordEventArgs eventArgs)
        {
            ...
            try
            {
                await _viewModel.GetBankInChequeList("HOLD");
        ...
        private async Task HoldBtn()
        {
            ...
            {
                _viewModel._typeBankIn = "HOLD";
                _viewModel.leTypeBank = CBT02300BankInChequeViewModel.TYPE_BANK_IM_CHEQUE.Hold;
        ...
        //{
        //    var loEx = new R_Exception();
        //    var loData = (List<CBT02300BankInChequeFrontDTO>)events.Data;

        //    if (loData.Count == 0)
        //    {
        //        var loErr = R_FrontUtility.R_GetError(typeof(Resources_CBT02300_Class), "Error_01");
        //        loEx.Add(loErr);
        //        events.Cancel = true;
        //    }
        //}
        private async Task ServiceSaveBatch(R_ServiceSaveBatchEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                CBT02300ProcessDataDTO poParam = new()
                {
                    ...
                    CACTION = lcAction,
                    CPROCESS_DATE = _viewModel.loParamaterFilter?.CDATE_FRONT?.ToString("yyyMMdd"),
                    CREASON = _viewModel.BankInChequeInfo.CREASON!
                };

                _viewModel.GetSelectedData(poParam, loList);

                if (await R_MessageBox.Show("Confirmation", $"Are you sure want to Hold selected cheque(s)?",
