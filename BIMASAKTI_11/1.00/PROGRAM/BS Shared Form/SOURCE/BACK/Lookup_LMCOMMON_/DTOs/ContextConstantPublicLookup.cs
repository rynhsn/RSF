using System;
using System.Collections.Generic;
using System.Text;

namespace Lookup_PMCOMMON.DTOs
{
    public class ContextConstantPublicLookup
    {
        public const string CCOMPANY_ID = "CCOMPANY_ID";
        public const string CUSER_ID = "CUSER_ID";
        public const string CPROPERTY_ID = "CPROPERTY_ID";
        public const string CCHARGE_TYPE_ID = "CCHARGE_TYPE_ID";
        public const string CCUSTOMER_TYPE = "CCUSTOMER_TYPE";
        public const string CDEPT_CODE = "CDEPT_CODE";
        public const string CAGGR_STTS = "CAGGR_STTS";
        public const string CTAXABLE_TYPE = "CTAXABLE_TYPE";
        public const string CACTIVE_TYPE = "CACTIVE_TYPE";
        public const string CTAX_DATE = "CTAX_DATE";
        public const string CTENANT_ID = "CTENANT_ID";
        public const string CTRANS_CODE = "CTRANS_CODE";
        public const string CPERIOD = "CPERIOD";
        public const string CCURRENCY_CODE = "CCURRENCY_CODE";
        public const string CLANGUAGE_ID = "CLANGUAGE_ID";
        public const string LNO_REMAINING = "LNO_REMAINING";
        public const string LHAS_REMAINING = "LHAS_REMAINING";
        public const string CINVGRP_CODE = "CINVGRP_CODE";
        public const string CBILLING_RULE_TYPE = "CBILLING_RULE_TYPE";
        public const string CUNIT_TYPE_CTG_ID = "CUNIT_TYPE_CTG_ID";
        public const string LACTIVE_ONLY = "LACTIVE_ONLY";
        //CR12 26/06/2024
        public const string CREF_NO = "CREF_NO";
        public const string CBUILDING_ID = "CBUILDING_ID";
        public const string CTRANS_STATUS = "CTRANS_STATUS";
        //CR17 11/10/2024
        public const string LACCRUAL = "LACCRUAL";
        public const string CLEASE_MODE = "CLEASE_MODE";
        public const string CINV_PRD = "CINV_PRD";

        public const string CSEQ_NO = "CSEQ_NO";
        //CR17 26/11/2024
        public const string CCATEGORY_ID = "CCATEGORY_ID";
        public const string CCALL_TYPE_ID= "CCALL_TYPE_ID";
        //CR18 28/11/2024
        public const string CCUSTOMER_ID = "CCUSTOMER_ID";
        public const string CRECEIPT_ID = "CRECEIPT_ID";
        public const string CLOI_AGRMT_ID = "CLOI_AGRMT_ID";
        public const string CREC_ID = "CREC_ID";
        //CR20 28/12/2024
        public const string CFLOOR_ID = "CFLOOR_ID";
        public const string CUNIT_ID = "CUNIT_ID";
        public const string CSUPERVISOR_ID = "CSUPERVISOR_ID";
        public const string LSUPERVISOR = "LSUPERVISOR";
        public const string CACTIVE_INACTIVE = "CACTIVE_INACTIVE";
        public const string CSTAFF_TYPE = "CSTAFF_TYPE";
        public const string CAPPLICATION = "CAPPLICATION";
        public const string CCLASS_ID = "CCLASS_ID";
        public const string CSTAFF_ID = "CSTAFF_ID";

    }
}
