...
namespace APF00100Model.ViewModel
{
    public class APF00110ViewModel : R_ViewModel<APF00110DTO>
    {
        ...
        public APF00100HeaderDTO loCallerTrxInfo = new APF00100HeaderDTO();
        ...
        public DateTime loLimitAllocationDate = DateTime.Today;

        public bool IsSingleCurrency = true;
        ...
        public async Task InitialProcess()
        {
            ...
            try
            {
                ...
                if (loAllocationEntryParameter.CTRANS_CODE != TransCodeConstant.VAR_PAYMENT_TO_SUPPLIER)
                {
        ...
        public async Task GetCallerTrxInfoAsync()
        {
            ...
            try
            {
                ...
                //if (!string.IsNullOrEmpty(loCallerTrxInfo.CREF_DATE))
                //{
                //    loCallerTrxInfo.DREF_DATE = DateTime.ParseExact(loCallerTrxInfo.CREF_DATE, "yyyyMMdd", null);
                //}
            }
            catch (Exception ex)
            {
        ...
        public async Task GetCAWTCustReceiptAsync()
        {
            ...
            try
            {
                loResult = await loAllocationListModel.GetCAWTCustReceiptAsync(new APF00100HeaderParameterDTO()
                {
                    CREC_ID = loAllocationEntryParameter.CREC_ID
                });
                loCallerTrxInfo = loResult.Data;
        ...
        public async Task GetCQCustReceiptAsync()
        {
            ...
            try
            {
                loResult = await loAllocationListModel.GetCQCustReceiptAsync(new APF00100HeaderParameterDTO()
                {
                    CREC_ID = loAllocationEntryParameter.CREC_ID
                });
                loCallerTrxInfo = loResult.Data;
        ...
        public void ValidationAllocation(APF00110DTO poParam)
        {
            ...
            try
            {
                ...
                llCancel = (poParam.NTARGET_ALLOCATION > 0) && (IsSingleCurrency) && (poParam.NTARGET_ALLOCATION != poParam.NCALLER_AMOUNT);
                if (llCancel)
                {
                    loEx.Add(R_FrontUtility.R_GetError(
                        typeof(Resources_Dummy_Class),
                ...
                llCancel = poParam.DREF_DATE < loLimitAllocationDate;
                if (llCancel)
                {
                    string monthName = string.Format(R_FrontUtility.R_GetMessage(typeof(Resources_Dummy_Class), "V013"), loLimitAllocationDate.ToString("dd-MMM-yyyy"));
        ...
        public async Task SaveAllocationAsync(APF00110DTO poEntity, eCRUDMode peCRUDMode)
        {
            ...
            try
            {
                ...
                if (loAllocationEntryParameter.CTRANS_CODE == TransCodeConstant.VAR_PURCHASE_RETURN || loAllocationEntryParameter.CTRANS_CODE == TransCodeConstant.VAR_PURCHASE_DEBIT_NOTE || loAllocationEntryParameter.CTRANS_CODE == TransCodeConstant.VAR_PURCHASE_DEBIT_ADJUSTMENT)
                {
                ...
                loParameter.CSUPPLIER_ID = loCallerTrxInfo.CSUPPLIER_ID;
                loParameter.CSUPPLIER_SEQ_NO = loCallerTrxInfo.CSUPPLIER_SEQ_NO;
                ...
                loAllocationEntryParameter.CALLOCATION_ID = loAllocationDetail.CREC_ID;
            }
            catch (Exception ex)
            {
                loException.Add(ex);
        ...
        public async Task DeleteAllocationAsync(APF00110DTO poEntity)
        {
            ...
            try
            {
                ...
                loAllocationEntryParameter.CALLOCATION_ID = "";
            }
            catch (Exception ex)
            {
                loException.Add(ex);
