    ...
    public class APF00100ViewModel : R_ViewModel<APF00100ListDTO>
    {
        ...
        public async Task GetAllocationListStreamAsync()
        {
            ...
            try
            {
                ...
                {
                    x.INO = a + 1;
                    if (!string.IsNullOrWhiteSpace(x.CALLOC_DATE))
                    {
        ...
        public async Task GetHeaderAsync()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(loHeader.CDOC_DATE))
                {
                    loHeader.DDOC_DATE = DateTime.ParseExact(loHeader.CDOC_DATE, "yyyyMMdd", null);
                }
                if (!string.IsNullOrWhiteSpace(loHeader.CREF_DATE))
                {
        ...
        public async Task GetCAWTCustReceiptAsync()
        {
            ...
            try
            {
                loResult = await loModel.GetCAWTCustReceiptAsync(new APF00100HeaderParameterDTO()
                {
                    CREC_ID = loAllocationParameter.CREC_ID
                });
                loHeader = loResult.Data;
        ...
        public async Task GetCQCustReceiptAsync()
        {
            ...
            try
            {
                loResult = await loModel.GetCQCustReceiptAsync(new APF00100HeaderParameterDTO()
                {
                    CREC_ID = loAllocationParameter.CREC_ID
                });
                loHeader = loResult.Data;
