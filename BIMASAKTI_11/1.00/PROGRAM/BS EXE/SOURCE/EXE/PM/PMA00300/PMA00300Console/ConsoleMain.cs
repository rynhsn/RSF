...
namespace PMA00300Console
{
    public class ConsoleMain
    {
        ...
        public async Task Main(bool plSingleTenant,
            R_MultiTenantDbEnumerationConstant.eDbConnectionStringType peConnectionType)
        {
            ...
            try
            {
                if (plSingleTenant)
                {
                    ...
                    {
                        _logger!.LogInfo(string.Format("Get Properties for tenant {0} and company {1}",
                            lcTenantNameDefault, loCompany.CCOMPANY_ID));
                        loProperties = await _GetPropertiesAsync(lcConnStrName, loCompany.CCOMPANY_ID);
                        _logger!.LogInfo(string.Format("Get Client Report Format for Company {0}",
                            loCompany.CCOMPANY_ID));
                        GetClientReportParameter = await PMA00300Cls.GetClientReportFormat(loCompany.CCOMPANY_ID, lcConnStrName);
                        ...
                        foreach (PropertyDTO loProperty in loProperties)
                        {
                            ...
                            _logger!.LogInfo(string.Format("Process Report Start For Property {0}",
                                loProperty.CPROPERTY_ID));
                            PMA00300Cls ProcessReport = new PMA00300Cls();
                            await ProcessReport.ProcessReport(GetClientReportParameter, poReportParam, lcConnStrName);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        //jika lotenant == FOUR-PPFSR lewati
                        // if (loTenant.CTENANT_ID == "FOUR-PPFSR")
                        // {
                        //     _logger.LogInfo(string.Format("Skip Tenant {0}", loTenant.CTENANT_ID));
                        //     continue;
                        // }

                        _logger!.LogInfo(string.Format("Get Connection Name for tenant {0}", loTenant.CTENANT_ID));
                        lcConnStrName =
                            await _multiTenantHelper.R_GetConnectionNameByTenantAsync(loTenant.CTENANT_ID,
                                peConnectionType);
                        ...
                        {
                            _logger!.LogInfo(string.Format("Get Properties for tenant {0} and company {1}",
                                loTenant.CTENANT_ID, loCompany.CCOMPANY_ID));
                            loProperties = await _GetPropertiesAsync(lcConnStrName, loCompany.CCOMPANY_ID);
                            _logger!.LogInfo(string.Format("Get Client Report Format for Company {0}",
                                loCompany.CCOMPANY_ID));
                            GetClientReportParameter = await PMA00300Cls.GetClientReportFormat(loCompany.CCOMPANY_ID, lcConnStrName);
