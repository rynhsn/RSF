...
{
    public class APR00600PrintSupplierIDController : R_ReportControllerBase
    {
        ...
        public APR00600PrintSupplierIDController(ILogger<LoggerAPR00600Print> logger)
        {
            LoggerAPR00600Print.R_InitializeLogger(logger);
            _logger = LoggerAPR00600Print.R_GetInstanceLogger();
            _activitySource = APR00600Activity.R_InitializeAndGetActivitySource(nameof(APR00600PrintSupplierIDController));
        ...
        {
            pcFileTemplate = System.IO.Path.Combine("Reports", "APR00600ReportSupplierId.frx");
        }
        ...
        public FileStreamResult SupplierId_ReportListGet(string pcGuid)
        {
            using Activity activity = _activitySource.StartActivity($"{GetType().Name}.{MethodBase.GetCurrentMethod().Name}");
