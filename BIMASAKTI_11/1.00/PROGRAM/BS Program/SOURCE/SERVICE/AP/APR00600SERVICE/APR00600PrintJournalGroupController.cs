...
{
    public class APR00600PrintJournalGroupController : R_ReportControllerBase
    {
        ...
        public APR00600PrintJournalGroupController(ILogger<LoggerAPR00600Print> logger)
        {
            LoggerAPR00600Print.R_InitializeLogger(logger);
            _logger = LoggerAPR00600Print.R_GetInstanceLogger();
            _activitySource = APR00600Activity.R_InitializeAndGetActivitySource(nameof(APR00600PrintJournalGroupController));
        ...
        {
            pcFileTemplate = System.IO.Path.Combine("Reports", "APR00600ReportJournalGroup.frx");
        }
        ...
        public FileStreamResult JournalGroup_ReportListGet(string pcGuid)
        {
            using Activity activity = _activitySource.StartActivity($"{GetType().Name}.{MethodBase.GetCurrentMethod().Name}");
        ...
        private APR00600ReportWithBaseHeaderDTO GeneratePrint(APR00600ReportParamDTO poParam)
        {
            ...
            try
            {
                ...
                var tempData = loCls.GetReportData(poParam);
                var str_to = R_Utility.R_GetMessage(typeof(APR00600BackResources.Resources_Dummy_Class), "to", loCultureInfo);
                loData.Data = tempData.Count>0?tempData: new List<APR00600DataResultDTO>();
