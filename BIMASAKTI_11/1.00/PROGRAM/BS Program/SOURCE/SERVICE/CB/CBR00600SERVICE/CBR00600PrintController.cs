...
{
    public class CBR00600PrintController : R_ReportControllerBase
    {
        ...
        #region instantiate
        public CBR00600PrintController(ILogger<CBR00600DTO> logger)
        {
            //Initial and Get Logger
            LoggerCBR00600.R_InitializeLogger(logger);
            _LoggerPrint = LoggerCBR00600.R_GetInstanceLogger();
            _activitySource = CBR00600ActivitySourceBase.R_InitializeAndGetActivitySource(nameof(CBR00600PrintController));
        ...
        {
            pcFileTemplate = System.IO.Path.Combine("Reports", "CBR00600.frx");
        }
        ...
        [HttpPost]
        public R_DownloadFileResultDTO AllPrintCBTransactionPost(CBR00600DTO poParameter)
        {
            using Activity activity = _activitySource.StartActivity("AllPrintCBTransactionPost");
            _LoggerPrint.LogInfo("Start AllPrintCBTransactionPost");
            ...
            try
            {
                ...
                // Set Guid Param 
                _LoggerPrint.LogInfo("Set GUID Param AllPrintCBTransactionPost");
            ...
            _LoggerPrint.LogInfo("End AllPrintCBTransactionPost");
            loException.ThrowExceptionIfErrors();
            return loRtn;
        }

        [HttpGet, AllowAnonymous]
        public FileStreamResult AllStreamPrintCBTransactionsGet(string pcGuid)
        {
            using Activity activity = _activitySource.StartActivity("AllStreamPrintCBTransactionsGet");
            R_Exception loException = new R_Exception();
            CBR00600PrintLogKeyDTO<CBR00600DTO> loResultGUID = null;
            _LoggerPrint.LogInfo("Start AllStreamPrintCBTransactionsGet");
            ...
            loException.ThrowExceptionIfErrors();
            _LoggerPrint.LogInfo("End AllStreamPrintCBTransactionsGet");
        ...
        private CBR00600ResultPrintDTO GenerateDataPrint(CBR00600DTO poParam)
        {
            ...
            try
            {
                ...
                var loReceiptColumn = AssignValuesWithMessages(typeof(CBR00600BackResources.Resources_Dummy_Class), loCultureInfo, loColumnReceiptObject);
                loRtn.Column.OfficialReceiptColoumn = (CBR00600OfficialReceiptColumnDTO)loReceiptColumn;

                //Assign Allocation Column
                CBR00600AllocationColumnDTO loColumnAllocationObject = new CBR00600AllocationColumnDTO();
                var loAllocationColumn = AssignValuesWithMessages(typeof(CBR00600BackResources.Resources_Dummy_Class), loCultureInfo, loColumnAllocationObject);
                loRtn.Column.AllocationColoumn = (CBR00600AllocationColumnDTO)loAllocationColumn;

                //Assign Allocation Column
                CBR00600JournalColumnDTO loColumnJournalObject = new CBR00600JournalColumnDTO();
                var loJournalColumn = AssignValuesWithMessages(typeof(CBR00600BackResources.Resources_Dummy_Class), loCultureInfo, loColumnJournalObject);
                loRtn.Column.JournalColoumn = (CBR00600JournalColumnDTO)loJournalColumn;
                ...
                if (poParam.LALLOCATION)
                {
                    ...
                    var loData = loCls.GetDataReport(poParam);
                    if (loData.Count > 0)
                    {
                        var loGrouping = loData
                        ...
                        .ToList();
                        loRtn.Data.Allocation = loGrouping;
                    }
                }

                if (poParam.LJOURNAL)
                {
                    _LoggerPrint.LogInfo("Call Method Journal Report");
                    poParam.CREPORT_TYPE = "JOURNAL";
                    var loJournalData = loCls.GetDataReport(poParam);
                    if (loJournalData.Count > 0)
                    {
                        var loGroupingJournal = loJournalData
                         .GroupBy(x => x.CCB_REF_NO) // Group by header key
                         .Select(g => new CBR00600JournalHeaderDTO
                         {
                             ...
                             DetailJournal = g.Select(d => new CBR00600JournalDetailDTO
                             {
                                 ...
                                 NCREDIT_AMOUNT = d.NCREDIT_AMOUNT,
                                 NDEBIT_AMOUNT = d.NDEBIT_AMOUNT,
                                 DREF_DATE = DateTime.TryParseExact(d.CREF_DATE, "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var ldRefDate) ? (DateTime?)ldRefDate : null,
                                 CCURRENCY_CODE = d.CCURRENCY_CODE,
                                 CDOCUMENT_DATE = d.CDOCUMENT_DATE,
                                 CGLACCOUNT_NO_NAME = d.CGLACCOUNT_NO_NAME,
                                 CDOCUMENT_NO = d.CDOCUMENT_NO,
                                 DDOCUMENT_DATE = DateTime.TryParseExact(d.CDOCUMENT_DATE, "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var ldDocumentDate) ? (DateTime?)ldDocumentDate : null,
                             }).ToList()
                         })
                         .ToList();
                        loRtn.Data.Journal = loGroupingJournal;
                    }
                }
            }
            catch (Exception ex)
