...
{
    public class CBR00600PrintJournalTransferInternalController : R_ReportControllerBase
    {
        ...
        #region instantiate
        public CBR00600PrintJournalTransferInternalController(ILogger<CBR00600DTO> logger)
        {
            //Initial and Get Logger
            LoggerCBR00600.R_InitializeLogger(logger);
            _LoggerPrint = LoggerCBR00600.R_GetInstanceLogger();
            _activitySource = CBR00600ActivitySourceBase.R_InitializeAndGetActivitySource(nameof(CBR00600PrintJournalTransferInternalController));
        ...
        {
            pcFileTemplate = System.IO.Path.Combine("Reports", "CBR00600JournalTransferInternal.frx");
        }
        ...
        [HttpPost]
        public R_DownloadFileResultDTO AllPrintJournalTransferInternalCBTransactionPost(CBR00600DTO poParameter)
        {
            using Activity activity = _activitySource.StartActivity("AllPrintJournalTransferInternalCBTransactionPost");
            _LoggerPrint.LogInfo("Start AllPrintJournalTransferInternalCBTransactionPost");
            ...
            try
            {
                ...
                // Set Guid Param 
                _LoggerPrint.LogInfo("Set GUID Param AllPrintJournalTransferInternalCBTransactionPost");
            ...
            _LoggerPrint.LogInfo("End AllPrintJournalTransferInternalCBTransactionPost");
            loException.ThrowExceptionIfErrors();
            return loRtn;
        }

        [HttpGet, AllowAnonymous]
        public FileStreamResult AllStreamPrintJournalTransferInternalCBTransactionsGet(string pcGuid)
        {
            using Activity activity = _activitySource.StartActivity("AllStreamPrintJournalTransferInternalCBTransactionsGet");
            R_Exception loException = new R_Exception();
            CBR00600PrintLogKeyDTO<CBR00600DTO> loResultGUID = null;
            _LoggerPrint.LogInfo("Start AllStreamPrintJournalTransferInternalCBTransactionsGet");
            ...
            loException.ThrowExceptionIfErrors();
            _LoggerPrint.LogInfo("End AllStreamPrintJournalTransferInternalCBTransactionsGet");
        ...
        private CBR00600ResultPrintDTO GenerateDataPrint(CBR00600DTO poParam)
        {
            ...
            try
            {
                ...
                //Assign Journal Column
                loRtn.Column = new CBR00600ColoumnDTO();
                CBR00600JournalColumnDTO loColumnJournalObject = new CBR00600JournalColumnDTO();
                var loColumnJournal = AssignValuesWithMessages(typeof(CBR00600BackResources.Resources_Dummy_Class), loCultureInfo, loColumnJournalObject);
                loRtn.Column.JournalColoumn = (CBR00600JournalColumnDTO)loColumnJournal;
                        ...
                        var loGroupingJournal = loJournalData
                         ...
                         .Select(g => new CBR00600JournalHeaderDTO
                         {
                             ...
                             DetailJournal = g.Select(d => new CBR00600JournalDetailDTO
                             {
                                 ...
                                 CDOCUMENT_NO = d.CGLACCOUNT_NO,
                                 CDOCUMENT_DATE = d.CDOCUMENT_DATE,
                                 DDOCUMENT_DATE = DateTime.TryParseExact(d.CDOCUMENT_DATE, "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var ldDocDate) ? (DateTime?)ldDocDate : null,
                             }).ToList()
                         })
                         .ToList();
