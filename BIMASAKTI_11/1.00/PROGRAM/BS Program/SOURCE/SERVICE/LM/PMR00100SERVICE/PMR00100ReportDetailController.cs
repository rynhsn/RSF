                ...
                }).Select(data3b => new LOOStatusDetail3DTO()
                {
                    ...
                    .Select(selectCharge => new LOOStatusDetailChargeDTO()
                    {
                        ...
                        }).Select(selectChargeUnit => new LOOStatusDetailChargeTypeUnitDTO()
                        {
                            ...
                            LOOStatusDetailChargeTypeUnitCharge = selectChargeUnit
                            .GroupBy(charge => new
                            {
                                charge.CCHARGE_DETAIL_CHARGE_NAME,
                                charge.CCHARGE_DETAIL_TAX_NAME,
                                charge.CCHARGE_DETAIL_START_DATE,
                                charge.CCHARGE_DETAIL_END_DATE,
                                charge.CCHARGE_DETAIL_TENURE,
                                charge.CCHARGE_DETAIL_FEE_METHOD,
                                charge.NCHARGE_DETAIL_FEE_AMOUNT,
                                charge.NCHARGE_DETAIL_CALCULATED_FEE_AMOUNT
                            }) // Group by the fields that define uniqueness
                            .Select(group => group.First()) // Select the first item from each group to avoid duplicates
                            .Select(charge => new LOOStatusDetailChargeTypeUnitChargeDTO()
                            {
