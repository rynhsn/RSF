...
[ApiController]
[Route("api/[controller]/[action]")]
public class GSM05000Controller : ControllerBase, IGSM05000Async
{
    private LoggerGSM05000 _logger;
    private readonly ActivitySource _activitySource;
    ...
    [HttpPost]
    public async Task<R_ServiceGetRecordResultDTO<GSM05000DTO>> R_ServiceGetRecord(R_ServiceGetRecordParameterDTO<GSM05000DTO> poParameter)
    {
        using var loActivity = _activitySource.StartActivity(nameof(R_ServiceGetRecord));
        _logger.LogInfo("Start - Get Transaction Code Record");
        R_Exception loEx = new();
    ...
    [HttpPost]
    public async Task<R_ServiceSaveResultDTO<GSM05000DTO>> R_ServiceSave(R_ServiceSaveParameterDTO<GSM05000DTO> poParameter)
    {
        using var loActivity = _activitySource.StartActivity(nameof(R_ServiceSave));
        _logger.LogInfo("Start - Save Transaction Code Entity");
        R_Exception loEx = new R_Exception();
    ...
    [HttpPost]
    public async Task<R_ServiceDeleteResultDTO> R_ServiceDelete(R_ServiceDeleteParameterDTO<GSM05000DTO> poParameter)
    {
        throw new NotImplementedException();
    }
    
    [HttpPost]
    public async IAsyncEnumerable<GSM05000GridDTO> GetTransactionCodeListStream()
    {
        using var loActivity = _activitySource.StartActivity(nameof(GetTransactionCodeListStream));
        _logger.LogInfo("Start - Get Transaction Code List");
        R_Exception loEx = new();
        List<GSM05000GridDTO> loResult = new();
        GSM05000ParameterDb loDbPar;
        GSM05000Cls loCls;
        ...
        try
        {
            ...
            _logger.LogInfo("Get Transaction Code List");
            loResult = await loCls.GetTransactionCodeListDbAsync(loDbPar);
        }
        catch (Exception ex)
        {
            loEx.Add(ex);
    ...
    [HttpPost]
    public async Task<GSM05000ListDTO<GSM05000DelimiterDTO>> GetDelimiterList()
    {
        using var loActivity = _activitySource.StartActivity(nameof(GetDelimiterList));
        _logger.LogInfo("Start - Get Delimiter List");
        R_Exception loEx = new();
        ...
        try
        {
            ...
            _logger.LogInfo("Get Delimiter List");
            loResult = await loCls.GetDelimiterListDbAsync(loDbPar);
            loRtn = new GSM05000ListDTO<GSM05000DelimiterDTO> { Data = loResult };
        }
        catch (Exception ex)
        {
    ...
    [HttpPost]
    public async Task<GSM05000ExistDTO> CheckExistData(GSM05000TrxCodeParamsDTO poParams)
    {
        using var loActivity = _activitySource.StartActivity(nameof(CheckExistData));
        _logger.LogInfo("Start - Check Exist Data");
        R_Exception loEx = new();
...
public interface IGSM05000Async : R_IServiceCRUDAsyncBase<GSM05000DTO>
{
    IAsyncEnumerable<GSM05000GridDTO> GetTransactionCodeListStream();
    Task<GSM05000ListDTO<GSM05000DelimiterDTO>> GetDelimiterList();
    Task<GSM05000ExistDTO> CheckExistData(GSM05000TrxCodeParamsDTO poParams);
}