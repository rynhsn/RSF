...
namespace GSM00300BACK
{
    ...
    public class GSM00300Cls : R_BusinessObjectAsync<CompanyParamRecordDTO>
    {
        private RSP_GS_MAINTAIN_COMP_PARAMResources.Resources_Dummy_Class _rsp = new();
        ...
        protected override Task R_DeletingAsync(CompanyParamRecordDTO poEntity)
        {
            throw new NotImplementedException();
        }

        protected override async Task<CompanyParamRecordDTO> R_DisplayAsync(CompanyParamRecordDTO poEntity)
        {
            using Activity activity = _activitySource.StartActivity(MethodBase.GetCurrentMethod().Name);
            R_Exception loEx = new R_Exception();
            CompanyParamRecordDTO loRtn = null;
        ...
        protected override async Task R_SavingAsync(CompanyParamRecordDTO poNewEntity, eCRUDMode poCRUDMode)
        {
            using Activity activity = _activitySource.StartActivity(MethodBase.GetCurrentMethod().Name);
            R_Exception loEx = new R_Exception();
            string lcQuery;
            ...
            try
            {
                ...
                loCmd.CommandText = lcQuery;
                loDb.R_AddCommandParameter(loCmd, "@CCOMPANY_ID", DbType.String, int.MaxValue, poNewEntity.CCOMPANY_ID);
                loDb.R_AddCommandParameter(loCmd, "@LPRIMARY_ACCOUNT", DbType.Boolean, int.MaxValue,
                    poNewEntity.LPRIMARY_ACCOUNT);
                loDb.R_AddCommandParameter(loCmd, "@CBASE_CURRENCY_CODE", DbType.String, int.MaxValue,
                    poNewEntity.CBASE_CURRENCY_CODE);
                loDb.R_AddCommandParameter(loCmd, "@CLOCAL_CURRENCY_CODE", DbType.String, int.MaxValue,
                    poNewEntity.CLOCAL_CURRENCY_CODE);
                loDb.R_AddCommandParameter(loCmd, "@CCENTER_BY", DbType.String, int.MaxValue, poNewEntity.CCENTER_BY);
                loDb.R_AddCommandParameter(loCmd, "@LCASH_FLOW", DbType.Boolean, int.MaxValue, poNewEntity.LCASH_FLOW);
                loDb.R_AddCommandParameter(loCmd, "@CACTION", DbType.String, int.MaxValue, lcAction);
                loDb.R_AddCommandParameter(loCmd, "@CUSER_ID", DbType.String, int.MaxValue, poNewEntity.CUSER_ID);
        ...
        public async Task<CheckPrimaryAccountDTO> CheckIsPrimaryAccountAsync(string pcCompanyId)
        {
            using Activity activity = _activitySource.StartActivity(MethodBase.GetCurrentMethod().Name);
            R_Exception loException = new R_Exception();
            CheckPrimaryAccountDTO loRtn = null;
            try
            {
                ...
                string lcQuery =
                    $"SELECT TOP 1 1 as LIS_PRIMARY FROM GSM_COMPANY (NOLOCK) WHERE CCOMPANY_ID <> @CCOMPANY_ID AND LPRIMARY_ACCOUNT = 1";
                DbCommand loCmd = loDb.GetCommand();
                loCmd.CommandText = lcQuery;
        ...
        public async Task<ValidateCompanyDTO> ValidateCompanyParamEditableAsync(string pcCompanyId, string pcUserId)
        {
            R_Exception loEx = new();
            ValidateCompanyDTO loRtn = new();
            try
