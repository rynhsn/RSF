...
{
    public class PMR02000GeneralCls
    {
        private PMR02000Logger _logger;

        private readonly ActivitySource _activitySource;

        public PMR02000GeneralCls()
        {
        ...
        public List<CategoryTypeDTO> GetCustomerTypeList(CategoryTypeParamDTO poParam)
        {
            using Activity activity = _activitySource.StartActivity(MethodBase.GetCurrentMethod().Name);
            R_Exception loEx = new R_Exception();
            List<CategoryTypeDTO> loRtn = null;
            R_Db loDB;
            DbConnection loConn;
            DbCommand loCmd;
            string lcQuery;
            try
            {
                ...
                ShowLogDebug(lcQuery, loCmd.Parameters);
                var loRtnTemp = loDB.SqlExecQuery(loConn, loCmd, true);
                loRtn = R_Utility.R_ConvertTo<CategoryTypeDTO>(loRtnTemp).ToList();
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        public List<TransactionTypeDTO> GetTransTypeList(TransactionTypeDTO poParam)
        {
            using Activity activity = _activitySource.StartActivity(MethodBase.GetCurrentMethod().Name);
            R_Exception loEx = new R_Exception();
            ...
            try
            {
                ...
                lcQuery = "[RSP_PM_GET_CUST_ACTIVITY_TRANS_CODE_LIST]";
                loCmd.CommandType = CommandType.StoredProcedure;
                loCmd.CommandText = lcQuery;
                ...
                ShowLogDebug(lcQuery, loCmd.Parameters);
                var loRtnTemp = loDB.SqlExecQuery(loConn, loCmd, true);
                loRtn = R_Utility.R_ConvertTo<TransactionTypeDTO>(loRtnTemp).ToList();
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
