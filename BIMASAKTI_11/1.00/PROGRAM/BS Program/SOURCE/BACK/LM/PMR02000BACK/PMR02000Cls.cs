...
namespace PMR02000BACK
{
    public class PMR02000Cls
    {
        ...
        //public PrintLogoResultDTO GetBaseHeaderLogoCompany(ReportParamDTO poParam)
        //{
        ...
        public PrintLogoResultDTO GetCompanyLogo(ReportParamDTO poParam)
        {
            using Activity activity = _activitySource.StartActivity(MethodBase.GetCurrentMethod().Name);
            var loEx = new R_Exception();
            PrintLogoResultDTO loResult = null;
        ...
        public PrintLogoResultDTO GetCompanyName(string pcCompanyId)
        {
            ...
            try
            {
                ...
                //Debug Logs
                _logger.LogDebug(string.Format($"EXEC RSP_GS_GET_COMPANY_INFO '{pcCompanyId}'"));
                var loDataTable = loDb.SqlExecQuery(loConn, loCmd, true);
                loResult = R_Utility.R_ConvertTo<PrintLogoResultDTO>(loDataTable).FirstOrDefault();
            }
            catch (Exception ex)
