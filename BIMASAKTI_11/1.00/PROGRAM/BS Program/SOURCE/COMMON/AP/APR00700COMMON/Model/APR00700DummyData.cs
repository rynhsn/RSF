...
namespace APR00700COMMON.Model
{
    public class APR00700DummyData
    {
        public static APR00700PrintDisplayDTO APR00700PrintDislpayWithBaseHeader()
        {
            APR00700PrintDisplayDTO loRtn = new APR00700PrintDisplayDTO();
            loRtn.BaseHeaderData = new BaseHeaderDTO()
            {
                CCOMPANY_NAME = "PT Realta Chakradarma",
                CPRINT_CODE = "APR00700",
                CPRINT_NAME = "Customer Ledger",
                CUSER_ID = "RCM",
            };
            loRtn.ReportData = new APR00700ReportDataDTO();
            loRtn.ReportData.Header = "PM";
            loRtn.ReportData.Title = "Customer Ledger";
            loRtn.ReportData.Column = new APR00700LabelDTO();
            loRtn.ReportData.HeaderParam = new APR00700ParamDTO()
            {
                CCOMPANY_ID = "C001",
        ...
        public static List<APR00700SPResultDTO> GenerateDummyData(int totalRecords = 10, int totalTenants = 5)
        {
            var dummyList = new List<APR00700SPResultDTO>();
            ...
            for (int i = 0; i < totalRecords; i++)
            {
                ...
                // Buat objek DTO
                var dto = new APR00700SPResultDTO
                {
                    CCOMPANY_ID = "BSI",
                    CPROPERTY_ID = "ASHMD" + (i % 2 + 1),
                    CSUPPLIER_ID = currentTenantId,
                    CSUPPLIER_NAME = $"Tenant Nama {currentTenantId.Substring(4)}",
            ...
            return dummyList.OrderBy(x => x.CSUPPLIER_ID).ThenBy(x => x.DREF_DATE).ToList();
        }
