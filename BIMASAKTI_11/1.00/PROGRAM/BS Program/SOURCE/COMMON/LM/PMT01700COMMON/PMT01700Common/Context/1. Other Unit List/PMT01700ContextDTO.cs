namespace PMT01700COMMON.Context._1._Other_Untit_List
{
    public class PMT01700ContextDTO
    {
        public const string CPROPERTY_ID = "CPROPERTY_ID";
        public const string CTRANS_CODE = "CTRANS_CODE";
        public const string TRANS_CODE = "TRANS_CODE";
        public const string CDEPT_CODE = "CDEPT_CODE";
        public const string CREF_NO = "CREF_NO";
        public const string CSEQ_NO = "CSEQ_NO";
        public const string CFLOOR_ID = "CFLOOR_ID";
        public const string CBUILDING_ID = "CBUILDING_ID";
        public const string CUNIT_ID = "CUNIT_ID";
        public const string CCHARGE_MODE = "CCHARGE_MODE";
        public const string CPROGRAM_CODE = "CPROGRAM_CODE";
        //CR Offer LIst 18/07/2024
        public const string CFROM_REF_DATE = "CFROM_REF_DATE";
        public const string LEVENT = "LEVENT";
        public const string CCANCELLED = "CCANCELLED";
    }
}
