...
namespace PMR00170COMMON
{
    public static class GenerateDataModel
    {
        ...
        public static PMR00170SummaryResultDTO DefaultDataSummary()
        {
            PMR00170SummaryResultDTO loReturn = new PMR00170SummaryResultDTO()
            {
                Title = "LOC Status",
                Header = new PMR00170DataHeaderDTO()
                {
                    PROPERTY = "Harco Mas",
                ...
                ColumnSummary = new PMR00170ColumnSummaryDTO(),
                Label = new PMR00170LabelDTO()
            };

            List<PMR00170DataSummaryDbDTO> loCollectionData = new List<PMR00170DataSummaryDbDTO>();
            ...
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    for (int k = 0; k < 4; k++)
                    {
                        loCollectionData.Add(new PMR00170DataSummaryDbDTO()
                        {
                            CTRANS_CODE = $"Code {i}",
        ...
        public static PMR00170SummaryResultWithHeaderDTO DefaultDataSummaryWithHeader()
        {
            PMR00170SummaryResultWithHeaderDTO loReturn = new PMR00170SummaryResultWithHeaderDTO();
            loReturn.BaseHeaderData = GenerateDataModelHeader.DefaultData().BaseHeaderData;
            loReturn.PMR00170SummaryResulDataFormatDTO = DefaultDataSummary();
            return loReturn;
        }
        #endregion

        #region Format DETAIL
        public static PMR00170DetailResultDTO DefaultDataDetail()
        {
            PMR00170DetailResultDTO loReturn = new PMR00170DetailResultDTO()
            {
                Title = "LOC Status",
                Header = new PMR00170DataHeaderDTO()
                {
                ...
                ColumnDetail = new PMR00170ColumnDetailDTO(),
                ParameterVisible = new PMR00170ParameterVisibleDTO()
                {
            ...
            List<PMR00170DataDetailDbDTO> loCollectionData = new List<PMR00170DataDetailDbDTO>();
                        ...
                        for (int l = 1; l < 3; l++) //UNIT
                        {
                            for (int m = 1; m < 2; m++) //ChargeType
                            {
                                for (int n = 1; n < 4; n++) //ChargeTypeUnit
                                {
                                    ...
                                    {
                                        loCollectionData.Add(new PMR00170DataDetailDbDTO()
                                        {
                                            CTRANS_CODE = $"Code {i}",
                                            CTRANS_NAME = $"TransName {i}",
                                            CSALESMAN_ID = $"ID{i}",
        ...
        public static PMR00170DetailResultWithHeaderDTO DefaultDataDetailWithHeader()
        {
            PMR00170DetailResultWithHeaderDTO loReturn = new PMR00170DetailResultWithHeaderDTO();
            loReturn.BaseHeaderData = GenerateDataModelHeader.DefaultData().BaseHeaderData;
            loReturn.PMR00170DetailResultDataFormatDTO = DefaultDataDetail();
            return loReturn;
        }
        #endregion
