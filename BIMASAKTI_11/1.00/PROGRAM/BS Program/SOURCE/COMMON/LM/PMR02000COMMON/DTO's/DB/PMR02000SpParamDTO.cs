namespace PMR02000COMMON.DTO_s
{
    public class PMR02000SpParamDTO
    {
        public string CCOMPANY_ID { set; get; }
        public string CPROPERTY_ID { set; get; }
        public string CUSER_ID { get; set; }
        public string CREPORT_TYPE { set; get; }
        public string CCURRENCY_TYPE_CODE { set; get; }
        public string CBASED_ON { get; set; }
        public string CFROM_CUSTOMER_ID { set; get; }
        public string CTO_CUSTOMER_ID { set; get; }
        public string CFROM_JRNGRP_CODE { set; get; }
        public string CTO_JRNGRP_CODE { set; get; }
        public string CPERIOD { set; get; }
        public string CREMAINING_BASED_ON { set; get; }
        public string CCUT_OFF_DATE { set; get; }
        public string CFR_DEPT_CODE { set; get; }
        public string CTO_DEPT_CODE { set; get; }
        public string CTRANS_CODE { set; get; }
        public string CTENANT_CATEGORY_ID { set; get; }
        public string CSORT_BY { set; get; }
        public string CCATEGORY_ID { get; set; }
        public string CCATEGORY_NAME { set; get; }
        public string CLANGUAGE_ID { set; get; }
    }
}
