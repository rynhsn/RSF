namespace FAT00100Common
{
    /// <summary>
    /// Context constants for FAT00100 streaming methods
    /// </summary>
    public static class ContextConstants
    {
        // GetComboPeriodMonth
        public const string CSOFT_PERIOD = "FAT00100_CSOFT_PERIOD";

        // GetAssetList
        public const string CFOREIGN_LANGUAGE = "FAT00100_CFOREIGN_LANGUAGE";
        public const string CDEPT_CODE = "FAT00100_CDEPT_CODE";
        public const string CTRANSACTION_CODE = "FAT00100_CTRANSACTION_CODE";
        public const string CREFERENCE_NO = "FAT00100_CREFERENCE_NO";
        public const string CSTATUS = "FAT00100_CSTATUS";
        public const string DUPDATE_DATE = "FAT00100_DUPDATE_DATE";

        // GetDataGrid
        public const string CSUPPLIER_ID = "FAT00100_CSUPPLIER_ID";
        public const string CPERIODFROM = "FAT00100_CPERIODFROM";
        public const string CPERIODTO = "FAT00100_CPERIODTO";
        public const string CTRANS_STATUS = "FAT00100_CTRANS_STATUS";

        // GetGSM_SUPPLIER_INFO and GetGSM_SUPPLIER_CONTACT
        public const string CINFO_SEQNO = "FAT00100_CINFO_SEQNO";

        // FAT0010002 - GetFAAcquisitionDetailAllocExpenPageList
        public const string CASSET_CODE = "FAT00100_CASSET_CODE";
        public const string CASSET_TRANS_SEQNO = "FAT00100_CASSET_TRANS_SEQNO";

        // FAT0010003 - GetDataGrid
        public const string PCFR_DEPT_CODE = "FAT00100_PCFR_DEPT_CODE";
        public const string PCFR_TRANSACTION_CODE = "FAT00100_PCFR_TRANSACTION_CODE";
        public const string PCFR_REFERENCE_NO = "FAT00100_PCFR_REFERENCE_NO";

        // FAT00100GetDeptLookupList
        public const string CPROGRAM_ID = "FAT00100_CPROGRAM_ID";

        // FAT00100GetTransAssetList
        public const string CREC_ID = "FAT00100_CREC_ID";
        public const string CREF_NO = "FAT00100_CREF_NO";

        // FAT00100GetStatusList
        public const string CAPPLICATION = "FAT00100_CAPPLICATION";
        public const string CCLASS_ID = "FAT00100_CCLASS_ID";
        public const string CREC_ID_LIST = "FAT00100_CREC_ID_LIST";

        // FAT00100GetTransExpAllocList
        public const string CPARENT_ID = "FAT00100_CPARENT_ID";
        public const string CTRANS_CODE = "FAT00100_CTRANS_CODE";
        public const string CASSET_TRANS_SEQ_NO = "FAT00100_CASSET_TRANS_SEQ_NO";
    }
}
