...
namespace GLR00300Front
{
    public partial class GLR00300
    {
        ...
        private async Task LostFocusLookupFromAccountAsync()
        {
            var loEx = new R_Exception();
        ...
        private async Task LostFocusLookupFromCenter()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(loGetData.CCENTER_CODE))
                {
                    ...
                    {
                        loGetData.CCENTER_CODE = loResult.CCENTER_CODE;
                        loGetData.CCENTER_NAME = loResult.CCENTER_NAME;
                    }
                }
            }
            catch (Exception ex)
        ...
        private async Task GenerateReport()
        {
            ...
            try
            {
                ...
                {
                    string? lcFormat = null;

                    bool llIsJournalAdjustmentModeSplit = loParam.CJOURNAL_ADJ_MODE_CODE == "S";
                    bool llIsPrintByCenter = _viewModelGLR00300._lPrintByCenter;
                    bool llIsPrintBudget = _viewModelGLR00300._lPrintBudget;

                    if (llIsJournalAdjustmentModeSplit && !llIsPrintByCenter && !llIsPrintBudget) lcFormat = "A";
                    else if (llIsJournalAdjustmentModeSplit && !llIsPrintByCenter && llIsPrintBudget) lcFormat = "B";
                    else if (!llIsJournalAdjustmentModeSplit && !llIsPrintByCenter && !llIsPrintBudget) lcFormat = "C";
                    else if (!llIsJournalAdjustmentModeSplit && !llIsPrintByCenter && llIsPrintBudget) lcFormat = "D";
                    else if (llIsJournalAdjustmentModeSplit && llIsPrintByCenter && !llIsPrintBudget) lcFormat = "E";
                    else if (llIsJournalAdjustmentModeSplit && llIsPrintByCenter && llIsPrintBudget) lcFormat = "F";
                    else if (!llIsJournalAdjustmentModeSplit && llIsPrintByCenter && !llIsPrintBudget) lcFormat = "G";
                    else if (!llIsJournalAdjustmentModeSplit && llIsPrintByCenter && llIsPrintBudget) lcFormat = "H";
                    ...
                    if (!string.IsNullOrEmpty(lcFormat))
                    {
                        ...
                        await _reportService.GetReport(
                            ...
                            "GL",
                            $"rpt/GLR00300ReportFormat{lcFormat}/AllTrialBalanceReportPost",
                            $"rpt/GLR00300ReportFormat{lcFormat}/AllTrialBalanceReportGet",
                            loParam);
                    }
                    else
                    {
        ...
        private async Task AfterOpen_PopupSaveAs(R_AfterOpenPopupEventArgs eventArgs)
        {
            ...
            GLR00300ParamDBToGetReportDTO loParam;
            bool loFlag = false;
            try
            {
                if (eventArgs.Success)
                {
                    ...
                    _viewModelGLR00300.ValidationFieldEmpty();
                    var lcPeriodYear = _viewModelGLR00300.PeriodYear.ToString();
                    loParam = new GLR00300ParamDBToGetReportDTO()
                    {
                        CCOMPANY_ID = _clientHelper.CompanyId,
                        CUSER_ID = _clientHelper.UserId,
                        CFROM_PERIOD_NO = _viewModelGLR00300.PeriodNo, //based on spec the value Period_No FROM and TO is same
