    ...
    public partial class PMR02000 : R_Page
    {
        ...
        public async Task ComboboxPropertyValueChanged(string poParam)
        {
            ...
            try
            {
                ...
                await _viewModel.GetCategoryTypeAsync(new CategoryTypeParamDTO() { CPROPERTY_ID = _viewModel.ReportParam.CPROPERTY_ID });
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
        ...
        private void Validation()
        {
            ...
            try
            {
                ...
                if (_viewModel._dataBasedOn == "C" && (string.IsNullOrWhiteSpace(_viewModel.ReportParam.CFROM_CUSTOMER_ID) || string.IsNullOrWhiteSpace(_viewModel.ReportParam.CTO_CUSTOMER_ID)))
                {
                    loEx.Add("", _localizer["_val_cust"]);
                }
                if (_viewModel._dataBasedOn == "J" && (string.IsNullOrWhiteSpace(_viewModel.ReportParam.CFROM_JRNGRP_CODE) || string.IsNullOrWhiteSpace(_viewModel.ReportParam.CTO_JRNGRP_CODE)))
                {
                ...
                if (_viewModel._enableFilterDept && (string.IsNullOrWhiteSpace(_viewModel.ReportParam.CFR_DEPT_CODE) || string.IsNullOrWhiteSpace(_viewModel.ReportParam.CTO_DEPT_CODE)))
                {
                ...
                if (_viewModel._enableFilterCustCtg && string.IsNullOrWhiteSpace(_viewModel.ReportParam.CTENANT_CATEGORY_ID))
                {
        ...
        private ReportParamDTO GenerateParam()
        {
            ...
            try
            {
                ...
                loRtn.CPROPERTY_NAME = _viewModel.properties.FirstOrDefault(x => x.CPROPERTY_ID == loRtn.CPROPERTY_ID).CPROPERTY_NAME ?? "";
                ...
                loRtn.CCUT_OFF_DATE = loRtn.CREMAINING_BASED_ON == "C" ? _viewModel._dateCutOff.ToString("yyyyMMdd") : "";
                ...
                loRtn.CDATA_BASED_ON_DISPLAY = _viewModel._dataBasedOn == "C" ? _localizer["_radio_Customer"] : _localizer["_radio_JournalGroup"];
                loRtn.CREMAINING_BASED_ON_DISPLAY = loRtn.CREMAINING_BASED_ON == "C" ? _localizer["_radio_CutoffRemaining"] : _localizer["_radio_LastRemaining"];
                loRtn.CREPORT_TYPE = _viewModel.ReportParam.CREPORT_TYPE;
                loRtn.CREPORT_TYPE_DISPLAY = _viewModel.ReportParam.CREPORT_TYPE == "S" ? _localizer["_radio_Summary"] : _localizer["_radio_Detail"];
                if (!string.IsNullOrWhiteSpace(loRtn.CCUT_OFF_DATE))
                {
                    loRtn.DDATE_CUTOFF = DateTime.ParseExact(loRtn.CCUT_OFF_DATE, "yyyyMMdd", new CultureInfo(_clientHelper.ReportCulture));
                }
                ...
                loRtn.CTRANSTYPE_FILTER_DISPLAY = loRtn.IS_TRANSTYPE_FILTER_ENABLED ? _viewModel._transTypeList.FirstOrDefault(x => x.CTRANS_CODE == _viewModel.ReportParam.CTRANS_CODE).CTRANS_CODE_NAME : "";
                loRtn.IS_CUSTCTG_FILTER_ENABLED = _viewModel._enableFilterCustCtg;
                loRtn.CUSTCTG_FILTER_DISPLAY = loRtn.IS_CUSTCTG_FILTER_ENABLED ? _viewModel._categoryTypeList.FirstOrDefault(x => x.CCATEGORY_ID == loRtn.CTENANT_CATEGORY_ID).CCATEGORY_NAME : "";
                ...
                loRtn.CJRNGRP_DISPLAY = _viewModel.ReportParam.CFROM_JRNGRP_NAME != _viewModel.ReportParam.CTO_JRNGRP_NAME ? $"{_viewModel.ReportParam.CFROM_JRNGRP_NAME} ({_viewModel.ReportParam.CFROM_JRNGRP_CODE}) - {_viewModel.ReportParam.CTO_JRNGRP_NAME} ({_viewModel.ReportParam.CTO_JRNGRP_CODE})" : $"{_viewModel.ReportParam.CFROM_JRNGRP_NAME} ({_viewModel.ReportParam.CFROM_JRNGRP_CODE})";
                loRtn.CCUSTOMER_DISPLAY = _viewModel.ReportParam.CFROM_CUSTOMER_ID != _viewModel.ReportParam.CTO_CUSTOMER_ID ? $"{_viewModel.ReportParam.CFROM_CUSTOMER_NAME} ({_viewModel.ReportParam.CFROM_CUSTOMER_ID}) - {_viewModel.ReportParam.CTO_CUSTOMER_NAME} ({_viewModel.ReportParam.CTO_CUSTOMER_ID})" : $"{_viewModel.ReportParam.CFROM_CUSTOMER_NAME} ({_viewModel.ReportParam.CFROM_CUSTOMER_ID})";
        ...
        private void BeforeOpen_lookupFromTenant(R_BeforeOpenLookupEventArgs eventArgs)
        {
            ...
            { CCOMPANY_ID = _clientHelper.CompanyId, CPROPERTY_ID = _viewModel.ReportParam.CPROPERTY_ID, CCUSTOMER_TYPE = "01", CUSER_ID = _clientHelper.UserId, CSEARCH_TEXT = "" };
        ...
        private async Task OnLostFocus_LookupFromTenant()
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(_viewModel.ReportParam.CFROM_CUSTOMER_ID))
                {
                    ...
                    _viewModel.ReportParam.CFROM_CUSTOMER_NAME = loResult.CTENANT_NAME ?? ""; //assign bind textbox name kalo ada
                }
                else
                {
                    _viewModel.ReportParam.CFROM_CUSTOMER_NAME = ""; //kosongin bind textbox name kalo gaada
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(_viewModel.ReportParam.CTO_CUSTOMER_ID))
                {
                    ...
                    var loParam = new LML00600ParameterDTO // use match param as GSL's dto, send as type in search texbox
                    {
                        ...
                        CCUSTOMER_TYPE = "01",
                        CUSER_ID = _clientHelper.UserId,
                        CSEARCH_TEXT = _viewModel.ReportParam.CTO_CUSTOMER_ID, // property that bindded to search textbox
                    };
                    ...
                    _viewModel.ReportParam.CTO_CUSTOMER_NAME = loResult.CTENANT_NAME; //assign bind textbox name kalo ada
                }
                else
                {
                    _viewModel.ReportParam.CTO_CUSTOMER_NAME = ""; //kosongin bind textbox name kalo gaada
        ...
        private async Task OnLostFocus_LookupFromDept()
        {
            ...
            try
            {
                ...
                {
                    LookupGSL00700ViewModel loLookupViewModel = new(); //use GSL's model
                    var loResult = await loLookupViewModel.GetDepartment(new GSL00700ParameterDTO // use match param as GSL's dto, send as type in search texbox
                    {
        ...
        private async Task OnLostFocus_LookupToDept()
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(_viewModel.ReportParam.CTO_DEPT_CODE))
                {
                    ...
                    var loResult = await loLookupViewModel.GetDepartment(new GSL00700ParameterDTO // use match param as GSL's dto, send as type in search texbox
                    {
                        CSEARCH_TEXT = _viewModel.ReportParam.CTO_DEPT_CODE, // property that bindded to search textbox
                    }); //retrive single record
