...
{
    public partial class PMT02130 : R_Page, R_ITabPage
    {
        [Inject] IJSRuntime JS { get; set; }
        [Inject] private R_ILocalizer<PMT02100FrontResources.Resources_Dummy_Class> _localizer { get; set; }

        private PMT02130ViewModel loViewModel = new PMT02130ViewModel();
        ...
        protected override async Task R_Init_From_Master(object poParameter)
        {
            ...
            try
            {
                loViewModel.VAR_FROM_DATE = new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1).AddMonths(-3);
                loViewModel.VAR_TO_DATE = DateTime.Today;

                loViewModel.lcStatusCode = "04";
        ...
        private void Before_Open_View_Checklist_Popup(R_BeforeOpenPopupEventArgs eventArgs)
        {
            eventArgs.Parameter = loViewModel.loHandover;
            eventArgs.TargetPageType = typeof(PMT02131);
        }

        private async Task After_Open_View_Checklist_Popup(R_AfterOpenPopupEventArgs eventArgs)
        {
        ...
        private async Task FromDate_ValueChanged(DateTime? poParam)
        {
            var loEx = new R_Exception();

            try
            {
                loViewModel.VAR_FROM_DATE = poParam;
                await _gridHandoverBuildingRef.R_RefreshGrid(null);
            }
            catch (Exception ex)
            {
        ...
        private async Task ToDate_ValueChanged(DateTime? poParam)
        {
            var loEx = new R_Exception();

            try
            {
                loViewModel.VAR_TO_DATE = poParam;
                await _gridHandoverBuildingRef.R_RefreshGrid(null);
            }
            catch (Exception ex)
            {
