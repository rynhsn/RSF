    ...
    public partial class GST00500 : R_Page
    {
        ...
        private async Task R_Before_BtnView(R_BeforeOpenDetailEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (programParts.Count() == 2)
                {
                    programNamespace = programParts[0];
                    programPage = programParts[1];
                }
                else if(programParts.Count() == 1)
                {
                    programNamespace = programParts[0];
                    programPage = programParts[0]; // Default page if not specified
                }
                else
                {
                    var loErr = R_FrontUtility.R_GetError(typeof(Resources_GST00500_Class), "Error_05");
                    loEx.Add(loErr);
                }
