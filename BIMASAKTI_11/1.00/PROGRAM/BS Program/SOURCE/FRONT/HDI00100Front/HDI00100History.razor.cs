...
public partial class HDI00100History : R_ITabPage
{
    ...
    protected override async Task R_Init_From_Master(object poParam)
    {
        ...
        try
        {
            await _viewModel.Init(poTabStatus: TabStatus.History, (string)poParam);
        }
        catch (Exception ex)
        {
            loEx.Add(ex);
    ...
    private void LostFocusFromDate(object obj)
    {
        var loDate = obj as DateTime? ?? DateTime.MinValue;
        if (loDate == DateTime.MinValue)
        {
            _viewModel.Param.DFROM_DATE = DateTime.Now;
        }
    }
    
    private void LostFocusToDate(object obj)
    {
        var loDate = obj as DateTime? ?? DateTime.MinValue;
        if (loDate == DateTime.MinValue)
        {
            _viewModel.Param.DTO_DATE = DateTime.Now;
        }
    }
}