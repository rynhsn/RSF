...
public partial class HDI00100 : R_Page
{
    ...
    private R_TabPage _tabNext { get; set; }
    private R_TabPage _tabHistory { get; set; }
    private R_TabStrip _tabStripRef { get; set; }
    ...
    protected override async Task R_Init_From_Master(object poParam)
    {
        ...
        try
        {
            await _viewModel.Init(poTabStatus: TabStatus.Active);
        }
        catch (Exception ex)
        {
            loEx.Add(ex);
    ...
    private async Task ValueChangedProperty(string value)
    {
        ...
        try
        {
            ...
            if (_viewModel.Param.CPROPERTY_ID != value)
            {
                _viewModel.Param.CPROPERTY_ID = value;
                _viewModel.Param.CBUILDING_ID = string.Empty;
                _viewModel.Param.CBUILDING_NAME = string.Empty;
                _viewModel.Param.CASSET_CODE = string.Empty;
                _viewModel.Param.CASSET_NAME = string.Empty;
                _viewModel.GridList.Clear();

                if (_tabStripRef.ActiveTab.Id == nameof(HDI00100Next))
                {
                    await _tabNext.InvokeRefreshTabPageAsync(_viewModel.Param.CPROPERTY_ID);
                }
                else if(_tabStripRef.ActiveTab.Id == nameof(HDI00100History))
                {
                    await _tabHistory.InvokeRefreshTabPageAsync(_viewModel.Param.CPROPERTY_ID);
                }
            }
        }
        catch (Exception ex)
    ...
    private void BeforeOpenTabNext(R_BeforeOpenTabPageEventArgs eventArgs)
    {
        eventArgs.Parameter = _viewModel.Param.CPROPERTY_ID;
        eventArgs.TargetPageType = typeof(HDI00100Next);
    }

    private void BeforeOpenTabHistory(R_BeforeOpenTabPageEventArgs eventArgs)
    {
        eventArgs.Parameter = _viewModel.Param.CPROPERTY_ID;
        eventArgs.TargetPageType = typeof(HDI00100History);
    }
}