...
public class PublicLookupHDRecordController : ControllerBase, IPublicHDLookupRecord
{
    ...
    [HttpPost]
    public HDLGenericRecord<HDL00500DTO> HDL00500GetRecord(HDL00500ParameterDTO poEntity)
    {
        using Activity activity = _activitySource.StartActivity("HDL00500GetRecord");
        var loEx = new R_Exception();
        HDLGenericRecord<HDL00500DTO> loRtn = new();
        _loggerHD.LogInfo("Start HDL00500GetRecord");

        try
        {
            var loCls = new PublicHDLookUpCls();
            _loggerHD.LogInfo("Set Param HDL00500GetRecord");
            poEntity.CCOMPANY_ID = R_BackGlobalVar.COMPANY_ID;
            poEntity.CUSER_ID = R_BackGlobalVar.USER_ID;
            var loResult = loCls.TaskLookup(poEntity);

            _loggerHD.LogInfo("Filter Search by text HDL00500GetRecord");
            loRtn.Data = loResult.Find(x =>
                x.CTASK_CODE.Equals(poEntity.CSEARCH_TEXT_ID.Trim(), StringComparison.OrdinalIgnoreCase));
        }
        catch (Exception ex)
        {
            loEx.Add(ex);
        ...
        _loggerHD.LogInfo("End HDL00500GetRecord");
        return loRtn;
    }
}