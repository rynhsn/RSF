...
[HttpPost]
    public IAsyncEnumerable<HDL00400DTO> HDL00400AssetLookup()
    {
        ...
        try
        {
            ...
            poParam.CAREA_TYPE = R_Utility.R_GetStreamingContext<string>(ContextConstantPublicLookup.CAREA_TYPE);
...
[HttpPost]
    public IAsyncEnumerable<HDL00500DTO> HDL00500TaskLookup()
    {
        using Activity activity = _activitySource.StartActivity(nameof(HDL00500TaskLookup));
        _loggerHD.LogInfo("Start HDL00500TaskLookup");
        var loException = new R_Exception();
        IAsyncEnumerable<HDL00500DTO> loRtn = null;

        try
        {
            var loCls = new PublicHDLookUpCls();
            var poParam = new HDL00500ParameterDTO();
                
            _loggerHD.LogInfo("Set Param HDL00500TaskLookup");
            poParam.CCOMPANY_ID = R_BackGlobalVar.COMPANY_ID;
            poParam.CUSER_ID = R_BackGlobalVar.USER_ID;
            poParam.CPROPERTY_ID = R_Utility.R_GetStreamingContext<string>(ContextConstantPublicLookup.CPROPERTY_ID);
            _loggerHD.LogInfo("Call Back Method GetTaskLookup");
            var loResult = loCls.TaskLookup(poParam);
                
            _loggerHD.LogInfo("Call Stream Method Data HDL00500TaskLookup");
            loRtn = GetStream<HDL00500DTO>(loResult);
        }
        catch (Exception ex)
        {
            loException.Add(ex);
        ...
        _loggerHD.LogInfo("End HDL00500TaskLookup");
        return loRtn;
    }
