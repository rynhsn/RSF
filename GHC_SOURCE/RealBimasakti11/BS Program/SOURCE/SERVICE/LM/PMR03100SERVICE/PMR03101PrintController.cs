...
using PMR03100SERVICE.DTO_s;

namespace PMR03100SERVICE
{
    public class PMR03101PrintController : R_ReportControllerBase
    {
        private Logger _logger;
        private R_ReportFastReportBackClass _ReportCls;
        private ParamDTO _Parameter;
        private readonly ActivitySource _activitySource;
        private PMR03100BackResources.Resources_Dummy_Class _backRes = new();

        public PMR03101PrintController(ILogger<Logger> logger)
        {
            Logger.R_InitializeLogger(logger);
            _logger = Logger.R_GetInstanceLogger();
            _activitySource = PMR03100Activity.R_InitializeAndGetActivitySource(nameof(PMR03101PrintController));
        ...
        {
            pcFileTemplate = System.IO.Path.Combine("Reports", "PMR03100.frx");
        }
        ...
        [HttpPost]
        public R_DownloadFileResultDTO DownloadResultPrintPost(ParamDTO poParameter)
        {
            using Activity activity = _activitySource.StartActivity($"{GetType().Name}.{MethodBase.GetCurrentMethod().Name}");
            _logger.LogInfo("Start - Post DownloadResultPrintPost Status");
            R_Exception loException = new();
            PrintLogKey loCache = null;
            R_DownloadFileResultDTO loRtn = null;
            try
            {
                loRtn = new R_DownloadFileResultDTO();
                loCache = new PrintLogKey
                {
                    poParam = poParameter,
            ...
            _logger.LogInfo("End - Print AR Collection");
            return loRtn;
        }

        [HttpGet, AllowAnonymous]
        public FileStreamResult GetFileStream_ARCollection(string pcGuid)
        {
            using Activity activity = _activitySource.StartActivity($"{GetType().Name}.{MethodBase.GetCurrentMethod().Name}");

            R_Exception loException = new();
            FileStreamResult loRtn = null;
            PrintLogKey loResultGUID = null;
            try
            {
                // Deserialize the GUID from the cache
                loResultGUID = R_NetCoreUtility.R_DeserializeObjectFromByte<PrintLogKey>(R_DistributedCache.Cache.Get(pcGuid));
                ...
                if (string.IsNullOrWhiteSpace(loResultGUID.poParam.CREPORT_FILENAME) || string.IsNullOrWhiteSpace(loResultGUID.poParam.CREPORT_FILEEXT))
                {
            ...
            loException.ThrowExceptionIfErrors();
            _logger.LogInfo("End - AR Collection Report Generation");
            return loRtn;
        }

        #region Helper

        private PrintDTO GenerateDataPrint(ParamDTO poParam)
        {
            using Activity activity = _activitySource.StartActivity($"{GetType().Name}.{MethodBase.GetCurrentMethod().Name}");
            ...
            try
            {
                ...
                LabelDTO loColumnObject = new();
                var loColumn = AssignValuesWithMessages(typeof(PMR03100BackResources.Resources_Dummy_Class), loCultureInfo, loColumnObject);

                // Set base header data
                _logger.LogDebug("Deserialized Print Parameters: {@PrintParameters}");

                loParam.CPRINT_CODE = ContextConstant.CPROGRAM_ID;
                loParam.CPRINT_NAME = ContextConstant.CPROGRAM_NAME;
                loParam.CUSER_ID = R_BackGlobalVar.USER_ID;
                var loCls = new PMR03100BACK.PMR03101Cls();
                ...
                ReportDataDTO loData = new ReportDataDTO()
                {
                    Title = ContextConstant.CPROGRAM_NAME,
                    Header = ContextConstant.CPROGRAM_NAME,
                    Label = (LabelDTO)loColumn,
                    Param = poParam,
                ...
                var loCollData = loCls.GetReportDataList(poParam);
                ...
                loData.Data = new List<SPResultDTO>(loCollData);
