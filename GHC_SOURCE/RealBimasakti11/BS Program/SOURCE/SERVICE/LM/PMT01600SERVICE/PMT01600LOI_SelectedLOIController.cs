...
namespace PMT01600Service
{
    ...
    [ApiController]
    public class PMT01600LOI_SelectedLOIController : ControllerBase, IPMT01600LOI_SelectedLOI
    {
        private readonly LoggerPMT01600? _loggerPMT01100;
        private readonly ActivitySource _activitySource;

        public PMT01600LOI_SelectedLOIController(ILogger<PMT01600LOI_SelectedLOIController> logger)
        {
            LoggerPMT01600.R_InitializeLogger(logger);
            _loggerPMT01100 = LoggerPMT01600.R_GetInstanceLogger();
            _activitySource = PMT01600Activity.R_InitializeAndGetActivitySource(nameof(PMT01600LOI_SelectedLOIController));
        }


        [HttpPost]
        public R_ServiceGetRecordResultDTO<PMT01600LOI_SelectedLOIDTO> R_ServiceGetRecord(R_ServiceGetRecordParameterDTO<PMT01600LOI_SelectedLOIDTO> poParameter)
        {
            string? lcMethod = nameof(R_ServiceGetRecord);
            _loggerPMT01100.LogInfo(string.Format("Start Method {0}", lcMethod));
            var loEx = new R_Exception();
            var loRtn = new R_ServiceGetRecordResultDTO<PMT01600LOI_SelectedLOIDTO>();

            try
            {
                _loggerPMT01100.LogInfo(string.Format("Initialize the loCls in method {0}", lcMethod));
                var loCls = new PMT01600LOI_SelectedLOICls();
        ...
        [HttpPost]
        public R_ServiceSaveResultDTO<PMT01600LOI_SelectedLOIDTO> R_ServiceSave(R_ServiceSaveParameterDTO<PMT01600LOI_SelectedLOIDTO> poParameter)
        {
            string? lcMethod = nameof(R_ServiceSave);
            using Activity activity = _activitySource.StartActivity(lcMethod)!;
            _loggerPMT01100.LogInfo(string.Format("Start Method {0}", lcMethod));
            R_Exception? loException = new R_Exception();
            R_ServiceSaveResultDTO<PMT01600LOI_SelectedLOIDTO> loReturn = new();
            try
            {
                _loggerPMT01100.LogInfo(string.Format("Initialize the loCls object as a new instance of Cls in method {0}", lcMethod));
                PMT01600LOI_SelectedLOICls? loCls = new();
        ...
        [HttpPost]
        public R_ServiceDeleteResultDTO R_ServiceDelete(R_ServiceDeleteParameterDTO<PMT01600LOI_SelectedLOIDTO> poParameter)
        {
