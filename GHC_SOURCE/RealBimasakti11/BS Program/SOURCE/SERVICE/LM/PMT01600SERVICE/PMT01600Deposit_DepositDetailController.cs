...
namespace PMT01600Service
{
    ...
    [ApiController]
    public class PMT01600Deposit_DepositDetailController : ControllerBase, IPMT01600Deposit_DepositDetail
    {
        private readonly LoggerPMT01600? _loggerPMT01100;
        private readonly ActivitySource _activitySource;

        public PMT01600Deposit_DepositDetailController(ILogger<PMT01600Deposit_DepositDetailController> logger)
        {
            LoggerPMT01600.R_InitializeLogger(logger);
            _loggerPMT01100 = LoggerPMT01600.R_GetInstanceLogger();
            _activitySource = PMT01600Activity.R_InitializeAndGetActivitySource(nameof(PMT01600Deposit_DepositDetailController));
        }


        [HttpPost]
        public R_ServiceGetRecordResultDTO<PMT01600Deposit_DepositDetailDTO> R_ServiceGetRecord(R_ServiceGetRecordParameterDTO<PMT01600Deposit_DepositDetailDTO> poParameter)
        {
            string? lcMethod = nameof(R_ServiceGetRecord);
            _loggerPMT01100.LogInfo(string.Format("Start Method {0}", lcMethod));
            var loEx = new R_Exception();
            var loRtn = new R_ServiceGetRecordResultDTO<PMT01600Deposit_DepositDetailDTO>();

            try
            {
                _loggerPMT01100.LogInfo(string.Format("Initialize the loCls in method {0}", lcMethod));
                var loCls = new PMT01600Deposit_DepositDetailCls();
        ...
        [HttpPost]
        public R_ServiceSaveResultDTO<PMT01600Deposit_DepositDetailDTO> R_ServiceSave(R_ServiceSaveParameterDTO<PMT01600Deposit_DepositDetailDTO> poParameter)
        {
            string? lcMethod = nameof(R_ServiceSave);
            using Activity activity = _activitySource.StartActivity(lcMethod)!;
            _loggerPMT01100.LogInfo(string.Format("Start Method {0}", lcMethod));
            R_Exception? loException = new R_Exception();
            R_ServiceSaveResultDTO<PMT01600Deposit_DepositDetailDTO> loReturn = new();
            try
            {
                _loggerPMT01100.LogInfo(string.Format("Initialize the loCls object as a new instance of Cls in method {0}", lcMethod));
                PMT01600Deposit_DepositDetailCls? loCls = new();
        ...
        [HttpPost]
        public R_ServiceDeleteResultDTO R_ServiceDelete(R_ServiceDeleteParameterDTO<PMT01600Deposit_DepositDetailDTO> poParameter)
        {
