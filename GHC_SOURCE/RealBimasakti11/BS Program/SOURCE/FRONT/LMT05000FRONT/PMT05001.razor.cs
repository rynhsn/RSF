...
namespace PMT05000FRONT
{
    public partial class PMT05001 : R_Page, R_ITabPage
    {
        ...
        {
            _agreementChrgDiscViewModel._AgreementChrgDiscProcessParam.CPROPERTY_ID =(string) poParam.ToString();
        ...
        private async Task BtnProcess_ClickAsync()
        {
            ...
            try
            {
                ...
                if (!loEx.HasError)
                {
                    var loMsg = await R_MessageBox.Show("", _localizer["_msg_processundo_success"], R_eMessageBoxButtonType.OK);
                }
                await _agreementChrgDiscViewModel.GetAgreementChrgDiscListAsync(_clientHelper.CompanyId);
            }
            catch (Exception ex)
